...
case class ComunicaFederatedStrategy(sources: Seq[Source]) extends StrategyRequest {
  ...
  val lSourcesDefinition : Future[List[SourceComunica]] =
    Future.sequence(sources.toList.collect {
      case source : Source if source.sourcePath == SourcePath.UrlPath =>
        Future { ComunicaRequestDriver.sourceFromUrl(source.path,source.mimetype) }
          .asInstanceOf[Future[SourceComunica]]
        case source : Source if source.sourcePath == SourcePath.Content =>
          ComunicaRequestDriver.sourceFromContent(source.content,source.mimetype)
            .asInstanceOf[Future[SourceComunica]]
