...
object UnravelSessionSubscribeEventTest extends TestSuite {
  ...
  def stepDiscoveryExecutor(unsubscribe : Boolean = false) = {
    ...
    val swr = sw.something("h1",_.out(URI("http://bb")))
      .select(List("h1"))
  ...
  def tests: Tests = Tests {
    ...
    test("DiscoveryRequestEvent ERROR_HTTP_REQUEST") {
      ...
      var stepDiscovery : Map[String,Boolean] = Map(
       ...
       val swr =
        UnravelSession(config).something("h1",_.out(URI("http://bb")))
        .select(List("h1"))
    ...
    test("MalformedQueryException -  ERROR_HTTP_REQUEST") {
      ...
      UnravelSession(conf).something("a",_.in("some:toto")).select(List("a"))
        .commit().raw
        .map( _=> assert(false))
        .recover( _ => {
          assert(true)
